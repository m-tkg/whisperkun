...
- **UI に表示する文字列を追加・変更したら必ず日英両方をローカライズする**。
  ローカライズ方式は「キー＝日本語ソース文字列」で、SwiftUI が自動で `Localizable.strings` を引く
  （`Text("...")` / `Toggle("...")` 等。補間 `Text("〜: \(x)")` は `%@` 形式キーになる）。
  追加した文言は `Resources/ja.lproj/Localizable.strings` と `Resources/en.lproj/Localizable.strings`
  の**両方**にエントリを足す。日本語側はキーと値が同一。
